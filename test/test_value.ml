...
let test_format_int64_renders_bare_digits () =
  Alcotest.(check string)
    ...
    (format_to_string (Value.Int64 42L));
  Alcotest.(check string)
    "negative int64 keeps its sign" "-7"
    (format_to_string (Value.Int64 (-7L)));
  Alcotest.(check string)
    "zero renders as 0" "0"
    (format_to_string (Value.Int64 0L))

let test_format_string_quotes_with_double_quotes () =
  Alcotest.(check string)
    "non-empty string is wrapped in double quotes" {|"Alice"|}
    (format_to_string (Value.String "Alice"));
  Alcotest.(check string)
    "empty string renders as a bare quote pair" {|""|}
    (format_to_string (Value.String ""))

let test_format_bool_renders_lowercase_keywords () =
  Alcotest.(check string)
    "true keyword" "true"
    (format_to_string (Value.Bool true));
  Alcotest.(check string)
    "false keyword" "false"
    (format_to_string (Value.Bool false))

let test_to_string_matches_format_output () =
  Alcotest.(check string)
    "Int64"
    (format_to_string (Value.Int64 42L))
    (Value.to_string (Value.Int64 42L));
  Alcotest.(check string)
    "String"
    (format_to_string (Value.String "Alice"))
    (Value.to_string (Value.String "Alice"));
  Alcotest.(check string)
    "Bool"
    (format_to_string (Value.Bool true))
