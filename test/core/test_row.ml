...
let test_unqualified_lookup_returns_position_and_field () =
  match Row.find_field users_row_kind (row_column_reference "id") with
  | Ok (position, field) ->
...
let test_unqualified_lookup_returns_position_for_later_field () =
  match Row.find_field users_row_kind (row_column_reference "active") with
  | Ok (position, field) ->
...
let test_unqualified_lookup_unknown_returns_error () =
  match Row.find_field users_row_kind (row_column_reference "missing") with
...
let test_unqualified_lookup_ambiguous_returns_error_naming_qualifiers () =
  match Row.find_field cross_product_row_kind (row_column_reference "id") with
...
let test_format_column_reference_bare_when_unqualified () =
  Alcotest.(check string)
    "unqualified" "id"
    (Row.format_column_reference (row_column_reference "id"))
