...
let test_single_column_projection () =
  let rows = project_users [ row_column_reference "name" ] in
  let expected =
    [
      [| Scalar.String "Alice" |];
      [| Scalar.String "Bob" |];
...
let test_multi_column_projection_in_field_order () =
  let rows =
    project_users [ row_column_reference "name"; row_column_reference "email" ]
  in
  let expected =
    [
...
let test_projection_reorders_columns () =
  let rows =
    project_users [ row_column_reference "email"; row_column_reference "id" ]
  in
  let expected =
    [
...
let test_projection_of_non_leading_column () =
  (* "active" is at field index 3. A miscached position would pull the wrong
     row element. *)
  let rows = project_users [ row_column_reference "active" ] in
  let expected =
    [
      [| Scalar.Bool true |];
      [| Scalar.Bool false |];
...
let test_projected_kind_has_requested_fields () =
  let projected_kind, _project_row =
    Projection.resolve users_kind
      [ row_column_reference "email"; row_column_reference "id" ]
  in
  let field_names =
...
let test_projected_kind_preserves_qualifiers () =
  let projected_kind, _project_row =
    Projection.resolve users_kind [ row_column_reference "name" ]
  in
  let qualifiers =
    List.map
...
let test_projected_kind_has_no_refinements () =
  let projected_kind, _project_row =
    Projection.resolve users_kind
      [ row_column_reference "id"; row_column_reference "name" ]
  in
  Alcotest.(check int)
...
let test_format_single_bare_column () =
  Alcotest.(check string)
    ...
    (format_to_string [ row_column_reference "name" ])
...
let test_format_multiple_columns_joined_by_commas () =
  Alcotest.(check string)
    ...
    (format_to_string
       [
         row_column_reference "id";
         row_column_reference "name";
         row_column_reference "active";
       ])
