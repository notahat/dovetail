...
| `Ast`       | `t = Relation_name \| Restrict \| Project \| CrossProduct \| Join \| Insert \| Type \| Scalar_literal \| Row_literal \| Relation_literal` | What the user typed, structured. No semantics yet. `Insert` is a regular operator in `t`; the parser enforces that it appears only as the terminal step of a pipeline. `Type` is the `\| type` inspection operator. `Scalar_literal`, `Row_literal`, and `Relation_literal` are the bare-value forms a pipeline can take instead of starting from a named relation. |
...
| `Logical`   | `t = Scan \| Restrict \| Project \| CrossProduct \| Relation_literal \| Insert \| Type_op \| Scalar_literal \| Row_literal` | Algebra: *what* the query computes, with no execution detail. `Restrict` is σ; `Project` is π; `CrossProduct` is ×. `Insert` carries its source subtree and the target table name. `Type_op` carries its input subtree and yields its input's relation type. `Scalar_literal` and `Row_literal` carry the bare-value pipeline sources whose output is a scalar or row rather than a relation. `Logical.required_access` walks the tree and returns `` `Read `` or `` `Write `` so the REPL can pick the right transaction. |
...
| `Term`      | `'tag t = Scalar_value \| Scalar_kind \| Row_value \| Row_kind \| Relation_value of 'tag Relation.t \| Relation_kind of Relation.kind` | Unified pipeline payload: the thing an evaluated plan hands back. Six arms, one per (rung × value/kind) cell of the type ladder. `Term.format` renders any arm so the REPL can print without caring which it got. |
