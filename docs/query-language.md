...
This section grows one query, one operator at a time. By the end
you'll have built the canonical multi-operator query -- a join with
a trailing projection -- and seen enough of the pipeline shape to
make the reference sections that follow feel familiar.

Every query reads against the fixture from the previous section, so
you can compare each transformation to what came before.
...
That's the working set of operators. The reference sections below
systematise each operator and the expression and projection
sublanguages.
