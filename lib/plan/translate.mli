...
val translate :
  catalog:(string -> Relation.kind option) -> Logical.t -> Physical.t
(** [translate ~catalog plan] rewrites [plan] into an equivalent physical plan.
    ...
    [catalog] is consulted by rewrites and the [Insert] arm both -- currently
    the [IndexLookup] rewrite (which needs the primary-key column to recognise
    the pattern) and [Insert] (which needs the target schema). A callback
    returning [None] for every name is harmless for queries; for an insert it
    surfaces as the "unknown table" error.
