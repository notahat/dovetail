(** Executor for data-definition statements.
    ...
    returning the matching {!Ddl.Statement.read_result} or
    {!Ddl.Statement.write_result}. The AST it consumes lives in {!Ddl.Statement}
    — the split keeps the AST vocabulary free of any storage dependency.
...
(** [execute_write environment transaction statement] runs a write DDL statement
    ...
    write via the standard exception path of
    {!Storage.Engine.with_write_transaction}. *)
