(** Data-definition AST: the statement universe.
    ...
    Today's constructors are [List_tables] and [Drop_table]. The [:create table]
    write form has been retired in favour of the pipe-form
    [<type-expr> | create table <name>] operator carried by {!Ast}; the
    introspection statement [:describe] was retired once the [type] pipe
    operator gave the surface language a uniform way to ask the same question.
