(** Canonical-form printer for DDL statements.
    ...
    comma on every column line (including the last), and the [primary key (...)]
    clause on its own closing line.

    Output strings carry embedded newlines for the multi-line form but do not
    end with a trailing newline -- callers (the REPL's describe renderer, the
    round-trip test) add one when they need it. *)
